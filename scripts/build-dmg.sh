#!/usr/bin/env bash
# Packages dist/TorroMail.app into a distribution DMG with a drag-to-Applications
# window layout. If signing credentials are set, the DMG itself is signed,
# notarized, and stapled so the downloaded container passes Gatekeeper cleanly.
#
...
# Mirrors build-macos-app.sh: no `--always`, so an untagged repo falls back to
...
dmg="dist/TorroMail-$VERSION.dmg"
echo "==> Building $dmg"
...
create-dmg \
    --volname "TorroMail $VERSION" \
    --window-size 540 380 \
    --icon-size 128 \
    --icon "TorroMail.app" 140 190 \
    --app-drop-link 400 190 \
    --hide-extension "TorroMail.app" \
    --no-internet-enable \
    "$dmg" "$staging"
...
( cd dist && shasum -a 256 "TorroMail-$VERSION.dmg" > SHA256SUMS.txt )
